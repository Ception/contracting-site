...
export default function Header() {
  ...
  return (
    <header className="relative fixed top-0 left-0 w-full bg-red-700 text-slate-200 z-50 transition-all duration-300 ease-in-out shadow-lg">
      <nav className="container mx-auto flex items-center justify-between p-4">
            ...
            (href) => (
              <Link key={href} href={href} passHref>
                <span
                  ...
                  className="cursor-pointer text-slate-200 hover:bg-slate-700 hover:text-white transition-colors duration-300 p-2 rounded-lg text-base block w-full text-left lg:text-center"
                >
        ...
        <Link href="#contact-us" passHref>
          <span className="cursor-pointer inline-block px-2 lg:px-4 py-1 lg:py-2 text-slate-200 text-sm lg:text-base md:text-lg font-medium bg-green-500 border border-transparent hover:bg-slate-700 focus:border-green-500 focus:border-opacity-40 focus:ring-4 focus:ring-green-500 focus:ring-opacity-40 rounded-lg transition duration-300 hidden lg:inline-block">
            Get in Touch
          </span>
        </Link>
      </nav>
